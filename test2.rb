...
# This progam calculates monthly payment amount for a loan
# Lynette Podkranic
...
def loan__zero?(num)
  prompt("Congratulations - you don't need a loan!") if num.zero?
end

def verify_loan_amt(num)
  integer? && float? && positive?
  break
end

def get_loan_amt(num)
  loop do
    prompt("Please enter a dollar amount you wish to borrow:")
    loan_amt = gets.chomp
    verify_loan_amt(loan_amt)
  end

  num = case amt
        when "0" then loan_zero?
        when loan_amt.empty? then prompt("You didn't enter anything - try again")
        else prompt("Oh no, that's not valid - try again")
end
